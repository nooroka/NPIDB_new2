...
f = open("listresdomains_sorted.txt","r")#отбираем хиты с подходящим разрешением
...
op.close()
op3.close()
process3 = subprocess.Popen(["sort", "-k1", "interaction_mode_domain_corr.txt"], stdout = subprocess.PIPE, stderr = subprocess.PIPE)
data3,data33 = process3.communicate()
with open('interaction_mode_domain_corr2.txt', 'w') as f:
    f.write(data3)
...
for line2 in op2:
    ...
    if str(line2[0]) in listres:
        ...
        if len(line2) > 1:
            ...
            list4.append("NULL")
op2.close()
op4 = open("familiesend_red.txt","r")
for line4 in op4:
    line4 = line4.strip()
    line4 = line4.split("\t",1)
