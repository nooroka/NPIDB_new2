...
#listofdictintkeys = [] #список цифра белка+его цепь из hbond
#listofdictintkeys2 = [] #cписок цифра белка+его цепь из hydrophobic
...
if filenorm in sorted(os.listdir(".")): 
    ...
    if "# File" in linefirst:
        for line in op:
            line2 = line.split() #line2[0] должен быть ДНК, line2[1] должен быть белок. но нужно предусмотреть случаи, когда наоборот
            if str.find(line, '#') == -1 and str.find(line, 'File') == -1 and str.find(line, 'Hydrogen') == -1 and str.find(line, 'Atom') == -1:    
                        ...
                        '''эта часть с linex и protein нужна для того, чтобы посмотреть, совпадают ли белки из файла со вторичной структурой (stride) и из файлв Hbond'''
                ...
                if str.find(line,'vertices:') ==-1 and str.find(line,'edges:') == -1 and line.isspace() == False: #если в строке нет vertices и при этом строка не пустая
                    ...
                    if len(line2) == 2:
                      ...
                      if line2[4] == '"WHITE:':
                            ...
                            os.chdir("/home/nooroka/lastintmode7/")
...
#if "1l9z.H_372-425.1l9z.pdb1.pdb"  in file:
 #   print(str(file)+" "+str(dictint))
...
#if "3l4j.A_690-1160.3l4j.pdb1.pdb" in file:
   # print(file)
...
'''
#print(str(file)+" "+"union1"+" "+str(set1.union(set2)))
print(str(file)+" "+"union2"+" "+str(set3.union(set4)))           
w = open("/home/nooroka/lastintmode9/resultendmode210320","a")
